# Makefile - Sentiric Platform Orkestratörü v1.0

# Varsayılan ortamı 'local' olarak ayarla
ENV ?= local
# Yapılandırma reposunun yerel yolunu belirt (yan yana klasörler varsayımı)
...
# Ana komutlar
up: sync-config
	@echo "🚀 Platform '$(ENV)' ortamı için başlatılıyor..."
	# Ortama özel yapılandırma dosyasını ve CONFIG_REPO_PATH'i dışarıdan enjekte et
	CONFIG_REPO_PATH=$(CONFIG_REPO_PATH) docker compose --env-file $(ENV_FILE) up -d --build --remove-orphans

down:
	@echo "🛑 Platform durduruluyor..."
	CONFIG_REPO_PATH=$(CONFIG_REPO_PATH) docker compose --env-file $(ENV_FILE) down --volumes

logs:
	@echo "📜 Loglar izleniyor... (Ctrl+C ile çık)"
	CONFIG_REPO_PATH=$(CONFIG_REPO_PATH) docker compose --env-file $(ENV_FILE) logs -f $(filter-out $@,$(MAKECMDGOALS))

ps:
	@echo "📊 Konteyner durumu:"
	CONFIG_REPO_PATH=$(CONFIG_REPO_PATH) docker compose --env-file $(ENV_FILE) ps
