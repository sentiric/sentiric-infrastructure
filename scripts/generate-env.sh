...
# doğru bir şekilde çözer ve nihai KEY=VALUE çiftlerini yazar.
...
while IFS= read -r line || [[ -n "$line" ]]; do
    # Satırın "source" ile başlayıp başlamadığını kontrol et
    if [[ $line == source* ]]; then
        # `source ../common/platform.env` gibi bir satırdan dosya yolunu al
        relative_path=$(echo "$line" | cut -d' ' -f2)
        
        # DÜZELTME: Yolu her zaman CONFIG_BASE_DIR'e göre çöz
        source_file_path="${CONFIG_BASE_DIR}/${relative_path}"
        
        if [ -f "$source_file_path" ]; then
            # Dosyadaki yorumları ve boş satırları atla, geri kalanını ekle
